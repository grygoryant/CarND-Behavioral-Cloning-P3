...
def preprocess_frame(img):
	...
	#ret_img = cv2.cvtColor(ret_img, cv2.COLOR_BGR2YUV)
	return ret_img
	...
	while 1:
		...
		for offset in range(0, num_samples, batch_size):
			...
			for batch_sample in batch_samples:
				...
				images.append(preprocess_frame(center_image))
				angles.append(center_angle)
				images.append(preprocess_frame(left_image))
				angles.append(left_angle)
				images.append(preprocess_frame(right_image))
				angles.append(right_angle)
				if abs(center_angle) > 0.3:
					center_image_mirrored = cv2.flip(center_image, 1)
					center_angle_mirrored = -center_angle
					images.append(preprocess_frame(center_image_mirrored))
				...
				if abs(left_angle) > 0.3:
					...
					images.append(preprocess_frame(left_image_mirrored))
				...
				if abs(right_angle) > 0.3:
					...
					images.append(preprocess_frame(right_image_mirrored))
